...
const Repository = async ({ params }: RepositoryModalProps) => {
    const { repository } = await gql
        .GetRepository({ owner: params.owner, name: params.repository })
        .catch(notFound);
                        ...
                        <Box>
                            ...
                            <Progress
                                ...
                                sections={repository.languages.edges.map((language) => {
                                    const value =
                                        // @ts-ignore
                                        (language.size / repository.languages.totalSize) * 100;

                                    return {
                                        value,
                                        color: language.node.color ?? 'gray',
                                        tooltip: `${language.node.name} ${
                            ...
                            <Flex columnGap='sm' mt='md' rowGap='5px' wrap='wrap'>
                                {repository.languages.edges.map((language) => {
                                    ...
                                    return (
                                        <Flex key={language.node.id} align='center' gap='3px'>
                                            <Box
                                                sx={{
                ...
                <Box mt='lg'>
                    ...
                    <Flex direction='column' gap='lg' mt='md'>
                        {repository.issues.nodes.map((issue) => (
                            <Card key={issue.id} padding='lg' pos='relative' radius='md'>
                                {issue.author && (
                                    <Tooltip label={issue.author.login}>
                                        <Avatar
                                            component='a'
                                            href={getRouteProfile(issue.author.login)}
                                            radius='lg'
                                            size='sm'
